...
export default function ChatInterface() {
  ...
  return (
    <div className="flex h-full flex-col bg-white dark:bg-slate-950">
      ...
      <div className="flex-1 overflow-y-auto">
        {isEmpty ? (
          ...
          <div className="flex h-full flex-col items-center justify-center px-4">
            ...
            >
              <div className="mx-auto mb-5 flex h-14 w-14 items-center justify-center rounded-full bg-slate-800 dark:bg-slate-800 dark:ring-1 dark:ring-neon-blue/30">
                <Sparkles className="h-7 w-7 text-white dark:text-neon-cyan" />
              </div>
              <h1 className="text-2xl font-semibold tracking-tight text-[#1f2328] dark:text-slate-100">
                {t('chat.title')}
              </h1>
              <p className="mt-2 text-[14px] text-[#656d76] dark:text-slate-400 max-w-md mx-auto leading-relaxed">
                {t('chat.subtitle')}
              </p>
                ...
                {SUGGESTIONS.map((s) => {
                  ...
                  return (
                    <motion.button
                      ...
                      className="group flex items-center gap-2 rounded-lg border border-[#d0d7de] bg-white px-3.5 py-2 text-[13px] font-medium text-[#1f2328] transition-all hover:border-[#0969da] hover:text-[#0969da] dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:border-neon-blue dark:hover:text-neon-blue"
                    >
                      <Icon className="h-4 w-4 text-[#656d76] group-hover:text-[#0969da] transition-colors dark:text-slate-500 dark:group-hover:text-neon-blue" />
                      {s.label}
                    </motion.button>
                  );
                })}
          ...
          <div className="mx-auto max-w-3xl px-4 py-6 space-y-6">
            ...
            {isGenerating && (
              ...
              >
                <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-slate-800 dark:bg-slate-800 dark:ring-1 dark:ring-neon-cyan/30">
                  <Sparkles className="h-3.5 w-3.5 text-white dark:text-neon-cyan" />
                </div>
      ...
      {/* ── Input area ── */}
      <div className="shrink-0 border-t border-[#d0d7de] bg-[#f6f8fa] px-4 pb-4 pt-3 dark:border-slate-700 dark:bg-slate-900">
        <form onSubmit={handleSubmit} className="mx-auto max-w-3xl">
          <div className="relative rounded-lg border border-[#d0d7de] bg-white shadow-sm transition-all focus-within:shadow-md focus-within:border-[#0969da] dark:border-slate-700 dark:bg-slate-900 dark:focus-within:border-neon-blue dark:focus-within:shadow-neon-blue/10">
            <textarea
              ref={inputRef}
              value={input}
              ...
              className="block w-full resize-none rounded-lg bg-transparent py-3 pl-4 pr-12 text-[14px] text-[#1f2328] placeholder-[#656d76] focus:outline-none dark:text-slate-100 dark:placeholder-slate-500"
            ...
            <button
              ...
              className="absolute bottom-2.5 right-2.5 flex h-7 w-7 items-center justify-center rounded-md bg-[#1f2328] text-white transition-all hover:bg-[#32383f] disabled:opacity-30 disabled:cursor-not-allowed dark:bg-neon-blue dark:text-slate-950 dark:hover:bg-neon-cyan"
            >
          ...
          <div className="mt-2 flex items-center gap-2">
            <div className="relative" ref={toneRef}>
              <button
                ...
                className="flex items-center gap-1.5 rounded-md border border-[#d0d7de] bg-white px-2.5 py-1 text-xs font-medium text-[#656d76] transition-colors hover:bg-[#f3f4f6] hover:text-[#1f2328] dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400 dark:hover:bg-slate-700 dark:hover:text-slate-200"
              >
              ...
              <AnimatePresence>
                {showToneMenu && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: 4 }}
                    className="absolute bottom-full left-0 mb-1 w-40 rounded-lg border border-[#d0d7de] bg-white py-1 shadow-lg z-50 dark:border-slate-700 dark:bg-slate-900"
                  >
                    {TONES.map((t) => (
                      <button
                        key={t}
                        ...
                        className={`block w-full px-3 py-1.5 text-left text-xs font-medium transition-colors ${
                          tone === t
                            ? 'bg-[#ddf4ff] text-[#0969da] dark:bg-neon-blue/10 dark:text-neon-blue'
                            : 'text-[#1f2328] hover:bg-[#f3f4f6] dark:text-slate-300 dark:hover:bg-slate-800'
                        }`}
                      >
                        {t}
                      </button>
              ...
              <AnimatePresence>
                {showCategoryMenu && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: 4 }}
                    className="absolute bottom-full left-0 mb-1 w-44 rounded-lg border border-[#d0d7de] bg-white py-1 shadow-lg z-50 dark:border-slate-700 dark:bg-slate-900"
                  >
                    {CATEGORIES.map((c) => (
                      <button
                        key={c}
            ...
            <span className="ml-auto text-[11px] text-[#656d76] dark:text-slate-500">
              {t('chat.shiftEnter')}
            </span>
          </div>
        </form>
...
function UserMessage({ content }) {
  return (
    <div className="flex items-start gap-3">
      <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#1f2328] text-[11px] font-bold text-white dark:bg-slate-700 dark:text-slate-200">
        JD
      </div>
      <div className="min-w-0 flex-1 pt-0.5">
        <p className="mb-1 text-[13px] font-semibold text-[#1f2328] dark:text-slate-100">You</p>
        <div className="text-[14px] leading-relaxed text-[#1f2328] dark:text-slate-300">
          {content}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="flex items-start gap-3">
      ...
      <div className="min-w-0 flex-1 space-y-2 pt-0.5">
        <p className="mb-1 text-[13px] font-semibold text-[#1f2328] dark:text-slate-100">PromptMaster</p>

        {/* Output card */}
        <div className="rounded-lg border border-[#d0d7de] bg-[#f6f8fa] px-4 py-3 dark:border-slate-700 dark:bg-slate-900">
          <pre className="whitespace-pre-wrap text-[14px] leading-relaxed text-[#1f2328] font-sans dark:text-slate-300">
            {msg.content}
          </pre>
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          {msg.meta && (
            <div className="flex items-center gap-2">
              <span className="rounded-full bg-[#ddf4ff] px-2 py-0.5 text-[11px] font-semibold text-[#0969da] dark:bg-neon-blue/10 dark:text-neon-blue">
                {msg.meta.category}
              </span>
              <span className="rounded-full bg-[#f6f8fa] px-2 py-0.5 text-[11px] font-medium text-[#656d76] border border-[#d0d7de] dark:bg-slate-800 dark:text-slate-400 dark:border-slate-700">
                {msg.meta.tone}
              </span>
              <span className="text-[11px] text-[#656d76] dark:text-slate-500">
          ...
          <div className="ml-auto flex items-center gap-1">
            <button
              ...
              className="flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-[#656d76] transition-colors hover:bg-[#f3f4f6] hover:text-[#1f2328] dark:text-slate-500 dark:hover:bg-slate-800 dark:hover:text-slate-200"
            >
              {isCopied ? (
                <>
                  <Check className="h-3.5 w-3.5 text-[#1a7f37] dark:text-neon-green" />
                  <span className="text-[#1a7f37] dark:text-neon-green">{t('chat.copied')}</span>
                </>
              ) : (
                <>
